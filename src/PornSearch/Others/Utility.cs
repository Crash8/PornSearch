// source : https://github.com/rolwincrasta/HTML5Decode/blob/master/HTML5Decode/Utility.cs
...
namespace HTML5Decode {
    public static class Utility {
        ...
        private const int UnicodePlane00End = 0x00FFFF;
        private const int UnicodePlane01Start = 0x10000;
        private const int UnicodePlane16End = 0x10FFFF;

        #region HtmlEncode / HtmlDecode methods

        private static readonly char[] SHtmlEntityEndingChars = { ';', '&' };
        ...
        private static void HtmlDecode(string value, StringBuilder output) {
            ...
            for (int i = 0; i < l; i++) {
                ...
                if (ch == '&') {
                    ...
                    int index = value.IndexOfAny(SHtmlEntityEndingChars, i + 1);
                    if (index > 0 && value[index] == ';') {
                        ...
                        if (entityLength > 1 && value[entityOffset] == '#') {
                            ...
                            if (parsedSuccessfully) {
                                ...
                                parsedSuccessfully = ((parsedValue < HighSurrogateStart) || (LowSurrogateEnd < parsedValue && parsedValue <= UnicodePlane16End));
                            }

                            if (parsedSuccessfully) {
                                if (parsedValue <= UnicodePlane00End) {
                                    // single character
        ...
        private static void ConvertSmpToUtf16(uint smpChar, out char leadingSurrogate, out char trailingSurrogate) {
            Debug.Assert(UnicodePlane01Start <= smpChar && smpChar <= UnicodePlane16End);

            int utf32 = (int)(smpChar - UnicodePlane01Start);
            leadingSurrogate = (char)((utf32 / 0x400) + HighSurrogateStart);
            trailingSurrogate = (char)((utf32 % 0x400) + LowSurrogateStart);
        }
        ...
        private static class HtmlEntities {
            ...
            static HtmlEntities() {
                ...
                Debug.Assert(SlookupTable.Count == Count, $"There should be {Count} HTML entities, but {nameof(SlookupTable)} has {SlookupTable.Count} of them.");
            }
#endif
            ...
            private static readonly Dictionary<string, string> SlookupTable =
            ...
            public static string Lookup(string entity) {
                string theString;
                SlookupTable.TryGetValue(entity, out theString);
                return theString;
            }
        }
    ...
    internal static class StringBuilderCache {
        ...
        public static StringBuilder Acquire(int capacity = 0) {
            if (capacity <= MaxBuilderSize) {
                StringBuilder sb = StringBuilderCache._cachedInstance;
                if (sb != null) {
                    // Avoid stringbuilder block fragmentation by getting a new StringBuilder
                    // when the requested size is larger than the current capacity
                    if (capacity <= sb.Capacity) {
                        StringBuilderCache._cachedInstance = null;
                        sb.Clear();
                        return sb;
                    }
                }

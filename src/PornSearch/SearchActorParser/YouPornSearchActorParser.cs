...
{
    internal class YouPornSearchActorParser : IPornSearchActorParser
    {
        private readonly IDocument _document;
        private readonly IElement _pagination;

        public YouPornSearchActorParser(IDocument document) {
            _document = document;
        ...
        public IEnumerable<IPornVideoThumbParser> GetVideoThumbs() {
            const string selector = "div.full-row-thumbs div.video-box";
            List<IHtmlDivElement> elements = _document.QuerySelectorAll<IHtmlDivElement>(selector).ToList();
            return elements.Select(div => new YouPornVideoActorThumbParser(div));
        }
    }
}
